...
class Dump:
    ...
    def __init__(
        ...
        self._intsize: int = 4
        self._offsize: int = 8
        # PicsellDois: Caching logic modif
    ...
    def load(self, path: str | os.PathLike) -> typing.Self:
        ...
        # PicsellDois: Caching logic modif
        path_cache = os.getcwd() + "/_Cache/" + str(path).replace(":","_").replace("/","_").replace("\\","_").replace(".","_") + "/"
        self._temp_dir = pathlib.Path(path_cache)
        is_cache_dir_found = os.path.exists(self._temp_dir)
        if not is_cache_dir_found :
    ...
    def table_data_writer(
        ...
        if entry.dump_id not in self._writers.keys():
            ...
            self._writers[entry.dump_id] = TableData(
                dump_id, self._temp_dir, self.encoding
            )
    ...
    def _tempfile(
        ...
        """
        # PicsellDois: Caching logic modif
        path = pathlib.Path(str(self._temp_dir) + f'/{dump_id}.gz')
